...
type Cube struct {
	Dimension     uint8          // Dimension of the cube (0 to 255)
	Configuration [5][5][5]uint8 // Three-dimensional array (5x5x5) using uint8
	Value         uint8          // Value to be calculated (0 to 255)
}

func NewCube() *Cube {
	cube := &Cube{
		Dimension: DIMENSION, // Set the dimension of the cube to 5x5x5
	}
...
// Evaluate the value of the cube
func (c *Cube) EvalValue() {
	c.Value = 0
	counter := uint8(0)
	tmp := uint16(0)
